...
def obtener_todas_las_filas(worksheet):
    ...
    try:
        ...
        print(f"❌ Error obteniendo filas: {e}")
        return []
...
def obtener_contactos_pendientes(worksheet):
    ...
    for registro in registros:
        try:
            ...
            print(f"⚠️ Error procesando registro para pendientes: {e}")
    return pendientes
...
def obtener_indices_columnas(worksheet):
    ...
    try:
        ...
        print(f"❌ Error obteniendo encabezados para índices: {e}")
    return indices
...
def columna_a_letra(col_num):
    ...
    try:
        ...
        print(f"⚠️ Error convirtiendo número de columna a letra: {e}")
    return letra
...
def actualizar_interes_alto_batch(worksheet, contactos, columnas_idx):
    try:
        ...
        if not all([col_interes, col_respondio, col_entrada_gratis]):
            print("❌ Columnas necesarias para actualizar interés no encontradas.")
            return
        ...
        print(f"❌ Error actualizando interés alto batch: {e}")
